...
export function Header() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50">
      ...
      <div className="bg-crimson text-white text-[11px] md:text-[12px]">
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-10 min-h-9 py-2 flex items-center justify-between gap-3 md:gap-5 flex-wrap md:flex-nowrap">
          <div className="hidden md:flex items-center gap-3 lg:gap-5 font-medium min-w-0 shrink">
            <a
              href="mailto:contact@inpoliticsinstitute.com"
              className="inline-flex items-center gap-1.5 hover:opacity-90 whitespace-nowrap"
            >
              <Mail className="size-3.5" /> contact@inpoliticsinstitute.com
            </a>
            <span className="opacity-60">|</span>
            <a
              href="tel:+33746440427"
              className="inline-flex items-center gap-1.5 hover:opacity-90 whitespace-nowrap"
            >
              <Phone className="size-3.5" /> +33 7 46 44 04 27
            </a>
            <span className="opacity-60">|</span>
            <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
              <MapPin className="size-3.5 shrink-0" /> Gigean · Montpellier Métropole
            </span>
            <span className="opacity-60">|</span>
            <span className="whitespace-nowrap">
              L'Institut des Décideurs Publics, de la Diplomatie, de la Performance Territoriale et
              de la Gouvernance Digitale.
            </span>
          </div>
          <div className="md:hidden flex items-center gap-2 font-medium w-full justify-center text-center">
            <Phone className="size-3.5 shrink-0" />
            <a href="tel:+33746440427" className="hover:opacity-90">
              +33 7 46 44 04 27
            </a>
            <span className="opacity-60">|</span>
      ...
      <div
        className={`transition-all ${scrolled || !isHome ? "glass-header" : "bg-white/85 backdrop-blur"}`}
      >
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 h-26 md:h-32 flex items-center justify-between gap-6">
          <Link
            to="/"
            className="logo-anim flex items-center shrink-0"
            aria-label="InPolitics Institute"
          >
            <img
              src={logo}
              alt="InPolitics Institute"
              className="h-24 md:h-28 lg:h-30 w-auto object-contain"
            />
          </Link>
          ...
          >
            <Link
              ...
              activeProps={{
                className:
                  "px-2 2xl:px-3 py-2 text-[12px] 2xl:text-[13px] font-semibold text-crimson whitespace-nowrap",
              }}
            >
              Accueil
            </Link>
        ...
        {mobile && (
          <div className="xl:hidden bg-white max-h-[80vh] overflow-y-auto shadow-lg">
            <div className="px-6 py-4 flex flex-col gap-1">
              <Link
                to="/"
                className="py-3 text-sm font-semibold text-anthracite hover:text-crimson"
              >
                Accueil
              </Link>
              {NAV.map((menu) => (
                  ...
                  <div className="pl-4 pb-2 flex flex-col gap-1">
                    {menu.items.map((it) => (
                      <Link
                        key={it.to}
                        to={it.to}
                        className="py-2 text-sm text-muted-foreground hover:text-crimson"
                      >
                        {it.label}
                      </Link>
                    ))}
